...
- **`games/*/index.html`** — play apps ("あそび"), limited by the shared daily 🎈 tickets. Currently: `games/pyoko/` (ぴょこっとタッチ, a whack-a-mole reaction/judgment game), `games/daruma/` (だるまさんがころんだ, a freeze-dance game where the phone is the game master — synthesized music plays / stops at random and the child dances / freezes away from the screen; no scoring, self-rated stars at the end), and `games/pitatto/` (ぴたっとストップ, a timing game: stop the moving frog on the lily pad, 10 tries per round, medals + bests in `pitatto-bests`).
- **`study/*/index.html`** — learning apps ("べんきょう"), **unlimited** — they never consume tickets. Currently: `study/numakasten/` (ヌマーカステン, a two-chamber counting-frame for number sense: free play + quiz mode with 4 levels), `study/tsuginani/` (つぎはな〜に？, a repeating-pattern app: duplicate → extend → abstract across 4 levels, following the task formats in the early-math patterning literature), and `study/yajirushi/` (やじるしめいろ, a Bee-Bot-style arrow-programming maze: level 1 moves one step per tap, levels 2–4 queue arrows then run them, adding obstacles and longer paths). Both persist the selected level in `localStorage` (`tsuginani-level` / `yajirushi-level`) and auto-level-up after a 5-⭐ streak.
