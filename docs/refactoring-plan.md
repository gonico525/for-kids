...
状況: **全7フェーズ実施済み**(2026-07-06、フェーズごとに1コミット)。
以下は計画時の内容に、実施時の判断(フェーズ4の viewport-fit 見送り等)を反映したもの。
...
- head メタの統一(study 3アプリに不足している manifest リンク・
  `apple-mobile-web-app-*`・apple-touch-icon を追加)
- `viewport-fit=cover` は**追加しない**: 左右の safe-area パディングを持たない
  既存アプリではノッチ被りを生む挙動変更になるため。新アプリはテンプレートが
  cover+safe-area 対応済み
